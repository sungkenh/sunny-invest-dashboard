...
주기적으로 실행해 데이터를 갱신하고 커밋합니다 → Netlify 자동 재배포.
...
1. `alpha-desk-web` 전체(이 `pipeline/`, `.github/`, `data/`, `*.html`, `netlify/` 포함)를
   Netlify에 연결된 GitHub 저장소에 푸시합니다.
...
- **저장소 구조**: 위 워크플로는 `alpha-desk-web` 내용이 **저장소 루트**에 있다고 가정합니다
...
- **빌드 횟수**: 6시간 주기 = 하루 4회 커밋·배포. Netlify 빌드분이 빠듯하면 주기를 늘리세요.
